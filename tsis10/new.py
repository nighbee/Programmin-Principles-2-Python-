...
cur.execute("CREATE TABLE  pp_phone_book (id SERIAL PRIMARY KEY, first_name VARCHAR(50) NOT NULL, last_name VARCHAR(50) NOT NULL, phone_number VARCHAR(20) NOT NULL)")
...
if new_first_name:
    cur.execute("UPDATE pp_phone_book  SET first_name = %s WHERE id = %s", (new_first_name, id))
if new_last_name:
    cur.execute("UPDATE  pp_phone_book SET last_name = %s WHERE id = %s", (new_last_name, id))
if new_phone_number:
    cur.execute("UPDATE  pp_phone_book SET phone_number = %s WHERE id = %s", (new_phone_number, id))

# Query data
cur.execute("SELECT * FROM  pp_phone_book")
rows = cur.fetchall()
for row in rows:
...
if delete_by == "name":
    ...
    cur.execute("DELETE FROM  pp_phone_book WHERE first_name = %s OR last_name = %s", (name, name))
elif delete_by == "phone":
    phone_number = input("Enter phone number to delete: ")
    cur.execute("DELETE FROM  pp_phone_book WHERE phone_number = %s", (phone_number,))
